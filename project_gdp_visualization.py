...
def reconcile_countries_by_name(plot_countries, gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    ...
    """
    list1=[];nam1={};nam2 ={};list2=[];list3=[]
    for i in plot_countries:
        nam1[plot_countries[i]] = i
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
   ...
   """
    ...
    for a in gdpinfo:
        ...
        if b != "":
            try:
                nam2[nam1[a]] = math.log10(float(b))
            except KeyError:
                pass
            except ValueError:
                pass
...
def new(plot_countries, gdp_countries, year):
    list1 = [];dic1 = {};dic2 = {}
    for i in plot_countries:
...
def render_world_map(name,gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    """
    Inputs:
    ...
    worldmap_chart.add(year, name)
    worldmap_chart.add("missing from world bank",gdpinfo)
    worldmap_chart.add("no date at this year", plot_countries)
...
fir=reconcile_countries_by_name(pygal_countries,reader)
sec=new(pygal_countries,reader,year)
render_world_map(build_map_dict_by_name(reader,pygal_countries,year),fir,sec,year,"isp_gdp_world_name_%s.svg"%(year))